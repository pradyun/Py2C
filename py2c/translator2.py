...
# NodeTransformer implements in itself.
class Python2ASTTranslator(object):
    ...
    def convert_to_python_node(self, node):
        ...
        py_node = getattr(NewAST, node_name) # I changed this from PyAST to NewAST.
        # I seemed to be unable to modify PyAST. The two are pretty much the same,
        # and it should be trivial to switch back if that's needed (probably).
...
class AST2CPPTranslator(object):
    def __init__(self):
        super(AST2CPPTranslator, self).__init__()
        print("CPPTranslator Initialized!")

        self.CPPString = """
        """ # for holding the body of the code

        self.ImportString = """
        """ # for holding necessary includes in the script

    def convert_to_CPP_code(self, code_node):        
        # recursively get the code; recursion is built into NewAST 
        self.CPPString = code_node.getCode();
        # imports have not been handled yet
        return(self.CPPString)

        
        pass

    def reset(self):
        # resets the ImportString and CPPString
...
#    print("    AST:", ast.dump(ast.parse(dedent(s))))
    ...
    translator2 = AST2CPPTranslator()
#    print("Returns:", end=" ")
    ASTNodes=translator.convert_to_python_node(ast.parse(dedent(s)))
    # Right now, convert to python node is translating to NewAST
    # Instead of PyAST. Should be trivial to switch back
    code = translator2.convert_to_CPP_code(ASTNodes)
    print(code)
#    translator2.convert_to_CPP_code(ASTNodes)
