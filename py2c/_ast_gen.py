...
PREFIX = dedent("""
    ...
    # To modify the contents of this file, modify them as this file may be
...
# Replicated to stay compatible with earlier Python 3 versions
...
class Node(object):
    ...
    def get_fields(self):
        retval = str(list(map(lambda x: x.name, self.attrs)))
        # Keeping a PEP-8 compliance
        if len(retval) > 64:
            retval = indent(retval[1:-1].replace(", ", ",\n"), " " * 8)
            retval = "[\n" + retval + "\n" + " " * 4 + "]"
        return retval
